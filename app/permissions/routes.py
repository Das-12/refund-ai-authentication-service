...
from .permissions import assign_permission_to_role,get_user_roles,assign_role_to_user,has_permission
...
async def list_permissions_for_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
...
async def list_permissions_for_user(username:str,token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    if not has_permission(request_user, "view_permissions_of_user"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Insufficient permissions",
        )
    ...
    return {"username": username, "roles": get_user_roles(user)}
